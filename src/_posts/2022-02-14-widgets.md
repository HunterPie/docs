...
- If you don't want to see the widget, you can disable the *Show Widget* option. This will hide the Widget but the Widget will still be created every time you open the game.
...
- Position: You can change the values of the **X** and **Y** coordinates for the widgets. Note that since it uses absolute positioning, if you set them to something out of boundaries, the widget will also be out of boundaries and will not be visible anymore.
...
HunterPie provides a way for the widgets to hide themselves whenever the game's HUD is visible. This setting is independent for each widget, so you will have to enable them individually for each widget you want to become invisible whenever the HUD is open.